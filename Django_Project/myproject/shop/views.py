...
def add_to_cart(request):
    if request.headers.get('x-requested-with')=='XMLHttpRequest':
        if request.user.is_authenticated:
            ...
            return  JsonResponse({'status': "Product Added to the Cart Successfully!!!"},status=200)
        else:
...
def remove_fav(request,fid):
    favitem=Favourite.objects.filter(id=fid)
    favitem.delete()
...
def product_details(request,cname,pname):
    if(Catagory.objects.filter(name=cname,status=0)):
       if(Products.objects.filter(name=pname,status=0)):
        ...
        messages.error(request,"No Such Object Found")
        return redirect("collections")
    else:
        messages.error(request,"No Such Object Found")
        return redirect("collections")